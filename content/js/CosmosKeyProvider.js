...
(function (CosmosKeyProvider) {
    ...
    if (typeof exports !== 'undefined') {
        if (typeof module !== 'undefined' && module.exports) {
            exports = module.exports = CosmosKeyProvider();
        }
        exports.CosmosKeyProvider = CosmosKeyProvider();
    }
    else {
        global.CosmosKeyProvider = CosmosKeyProvider();
    }
}).call(this, function () {
    ...
    var CosmosKeyProvider = function (regId, setupNeededCallback, appName) {
        if (typeof regId !== "string" || regId.length === 0) {
            throw new Error(
                "CosmosKeyProvider: regId must be a non-empty string");
        }

        if (typeof setupNeededCallback !== "function") {
            throw new Error(
                "CosmosKeyProvider: setupNeededCallback must be a function");
        }
    ...
    CosmosKeyProvider.prototype = Object.create(Object.prototype);
    CosmosKeyProvider.prototype.constructor = CosmosKeyProvider;
     ...
     */
    CosmosKeyProvider.prototype.setup = function () {
        ...
        // this regId as well.
        return $.when(regIdStore(m_regId))
            .then(function () {
            ...
            /*.catch(function (error) {
                ...
                // We aren't setup.  The most common case where we aren't setup is when
                // this a brand new user and we have no key store.  So, start out by
                // trying to write our regId to the privateKeyStore.
     ...
     */
    CosmosKeyProvider.prototype.getProfileKeys = function () {
        ...
        var promiseObj = new Promise(function (fullfill, reject) {
            $.ajax({
                ...
                if ($.isEmptyObject(data)) {
                    ...
                    response.privateEncryptionKey = BBMEnterprise.Util.b2array(data.privateEncryptionKey);
                    response.publicSigningKey = BBMEnterprise.Util.b2array(data.publicSigningKey);
                    response.privateSigningKey = BBMEnterprise.Util.b2array(data.privateSigningKey);
                    response.publicEncryptionKey = BBMEnterprise.Util.b2array(data.publicEncryptionKey);
                    fullfill(response);
                }
       ...
       /* var result = {};
        ...
        /*if (!m_setupComplete) {
            ...
            return rejected("CosmosKeyProvider is not setup");
        }
        ...
        return privateKeyStore().once("value")
            .then(function (privateKeys) {
                ...
                if (!privateKeys.val()) {
                    ...
                    m_setupNeededCallback({ reason: "NOT_SETUP" });
                    return rejected("CosmosKeyProvider no keys available");
                }
                if (privateKeys.val().regId !== m_regId) {
                    // Let the application know that the key store is associated with a
                    // different regId.
                    m_setupNeededCallback({
                        reason: "REGID_CONFLICT",
                        regId: privateKeys.val().regId
                    });
                    return rejected("CosmosKeyProvider regId conflict");
                }
                ...
                try {
                    ...
                    return rejected("CosmosKeyProvider failed to decode private "
            ...
            .then(function (publicKeys) {
                ...
                try {
                    ...
                    return rejected("CosmosKeyProvider failed to decode public "
                        + "encryption key: " + error);
                }
                try {
     ...
     * @returns void
     */
    CosmosKeyProvider.prototype.saveProfileKeys = function (keys) {


        /*var publicKey = {};
        publicKey.encryptionKey = "";
        publicKey.signingKey = "";
        publicKey.regId = m_regId;
        $.ajax({
            url: '/createKeyStore',
            type: 'post',
            dataType: 'json',
     ...
     *   getChatKey().
     */
    CosmosKeyProvider.prototype.getChatKeys = function () {
            ...
            $.ajax({
                ...
                if ($.isEmptyObject(data)) {
                    ...
                    for (var i = 0; i < arrayLength; i++) {
                        var curKeyPair = data[i];
                        var chatId;
                        var key;

                        try { chatId = BBMEnterprise.Util.base64urlDecode(curKeyPair.chatId); }
                        catch (error) {
                            log("Ignoring key for chatId=" + id
                                + "; failed to decode value; error=" + error);
                            return;
                        }
                        try { key = BBMEnterprise.Util.b2array(curKeyPair.chatKey); }
                        catch (error) {
     ...
     */
    CosmosKeyProvider.prototype.getChatKey = function (chatId) {
            ...
            $.ajax({
                ...
                if ($.isEmptyObject(data)) {
                    ...
                    try { id = BBMEnterprise.Util.base64urlEncode(chatId); }
                        ...
                        return rejected("CosmosKeyProvider failed to encode chatId="
                            + chatId + ": " + error);
                    }
                    for (var i = 0; i < arrayLength; i++) {
                        var curKeyPair = data[i];
                        if (id == curKeyPair.chatId) {
                            result = BBMEnterprise.Util.b2array(curKeyPair.chatKey);
                        }
                    }
                    fullfill(result);
                }
            ...
            .then(function (chatKey) {
                if (!chatKey.val()) {
                    ...
                    throw new BBMEnterprise.error.NotFoundError(
                        "CosmosKeyProvider no chatKey for chatId=" + chatId);
                }
                ...
                try { return BBMEnterprise.Util.b2array(chatKey.val()); }
                    ...
                    return rejected("CosmosKeyProvider failed to decode key for chatId="
                        + chatId + ": " + error);
                }
            });*/
    }
     ...
     * @returns void
     */
    CosmosKeyProvider.prototype.saveChatKey = function (chatId, key) {
     ...
     * @returns void
     */
    CosmosKeyProvider.prototype.removeChatKey = function (chatId) {
        // Make sure we are setup.
        if (!m_setupComplete) {
            m_setupNeededCallback({ reason: "NOT_SETUP" });
            return;
     ...
     *   be verifiable.
     */
    CosmosKeyProvider.prototype.getPublicKeys = function (regId) {
        ...
        var promiseObj = new Promise(function (fullfill, reject) {
            ...
            $.ajax({
                ...
                if ($.isEmptyObject(data)) {
                    ...
                    //
                    //result.privateEncryptionKey = BBMEnterprise.Util.b2array(data.privateEncryptionKey);
                    result.signingKey = BBMEnterprise.Util.b2array(data.publicSigningKey);
                    //result.privateSigningKey = BBMEnterprise.Util.b2array(data.privateSigningKey);
                    result.encryptionKey = BBMEnterprise.Util.b2array(data.publicEncryptionKey);
                    fullfill(result);
                }
        ...
        /*return publicKeyStore(regId).once("value")
            .then(function (publicKeys) {
                if (!publicKeys.val()) {
                    return rejected("CosmosKeyProvider no public keys for regId="
                        + regId);
                }
                ...
                try {
                    ...
                    log("Failed do decode the public encryption key="
                        ...
                        + "; error=" + error);
                    return rejected("CosmosKeyProvider failed to decode the public "
    ...
    function log(message) {
        console.log("CosmosKeyProvider: ", message);
    }
    ...
    // Return the CosmosKeyProvider object as the public interface.
    return CosmosKeyProvider;
});
