...
var curSentiment = 0;
var translatedMessageCount = 0;
var addingExistingChat = false;
    ...
    if (!gUser) {
        ...
        try {
            ...
            messenger = new BBMEnterprise.Messenger({
                ...
                getKeyProvider: getCosmosKeyProvider,
            });
        } catch (error) {
            console.log("Failed to init BBM SDK: " + error + " in UA=" + navigator.userAgent);
            if (error instanceof BBMEnterprise.error.BrowserNotSupportedError) {
        ...
        var setupComplete = new Promise(function (resolve, reject) {
            ...
            messenger.setup().then(function (registrationInfo) {
                if (registrationInfo.state === "Success") {
                    ...
                    //put google user ID to localStorage to decide if the login screen needs to show up when localStorage is expired
                    //
                    localStorage.userID = aadUser.oid;

                    //set the values to the local variables used by the web page.
                    userRegId = registrationInfo.regId;
                    userPin = registrationInfo.pin;
                    googleAuthToken = getGoogleAuthToken;
                    googleUserName = aadUser.displayName;
                    googleUserEmail = aadUser.upn;
                    googleUserAvatarImageURL = aadUser.avatarUrl;
...
function getCosmosKeyProvider(regId) {
    // Sign-in to our Firebase project using the already logged in google user's
    // credentials.
    //
    console.log("Hello cosmos");
    return $.when(
        keyProvider = new CosmosKeyProvider(
            regId, function (data) {
                console.error(data);
                document.getElementById("loginStatus").innerHTML
                    = "Failed to create a key provider.";
            ...
            console.log("Returning Key Provider");
                wait(3000);
                return keyProvider;
        })

    console.log("End Cosmos Key Provider");
}
...
function initContactList() {
    ...
    var user = {};
    user.regId = userRegId;
    user.oid = aadUser.oid;
    user.avatarUrl = "https://millscommunitysupport.files.wordpress.com/2013/08/generic-head-silhouette.jpg";
    user.name = aadUser.displayName;
    user.email = aadUser.upn;
    $.ajax({
        url: '/createUser',
        type: 'post',
        dataType: 'json',
    ...
    $.ajax({
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].user.regId != user.regId) {
                var cur = data[i].user;
                var regId = cur.regId;
                var name = cur.name;
                var key = cur.oid;
                var image = cur.avatarUrl;
                ...
                contactData.set(DATA_CONTACT_IMG_URL, image);
...
function showTab(tabName) {
    $(".tabPane").width("320");
    if (tabName === "fileViewer") {
        $('#listHeaderText').text("File Viewer");
        $(".tabPane").width("1000");
    ...
    if (tabName === "filePicker") {
        $('#listHeaderText').text("File Picker");
...
function createChatMessageElement(chatData) {
    if (chatData.get(DATA_CHAT).state === BBMEnterprise.Chat.State.Defunct) {
        ...
        return '<div class="chatMessage"></div>';
    }
}
...
function createIncomingMessageBubbleTemplate(chatId, messageId, messageContent) {
    ...
    $.ajax({
        ...
        type: 'post',
        dataType: 'json',
        data: { "message": chatMessage.content }
    }).done(function (data) {
        var i = 0;
        
        });*/

   /* $.post("/translate", { message: chatMessage.content }, function (data) {
   ...
   /* $.ajax({
    ...
    translatedMessageCount = translatedMessageCount + 1;
    TranslateText(chatMessage.content, translatedMessageCount);
    try {
        if (chatMessage.content.includes("Secure File Sent")) {
            var split = chatMessage.content.split(":");
            var documentGuid = split[1].trim();
            getTokenAndDisplay(documentGuid);
        }
    } catch (err) { }

    GetSentiment(chatMessage.content);
    ...
    var bubbleElement = '<div class="incomingMessageBubble"'
        ...
        + messageContent + '<div class="translated" id="translate' + translatedMessageCount.toString() + '">' + translatedMessage + '</div>'
...
function createCoversationBannerElement(chatId, defaultChatAvatar, defaultSubject) {
    ...
    return '<div class="bannerImageWrapper" id="bannerImageWrapper">'
        ...
        + '</div>'
        + '<div id="poweredByAzure" class="poweredByAzureWrapper">'
        + '<div class="progress azMenuItem"> '
        + '<div id="sentimentProgress" class="progress-bar bg-success" role= "progressbar" style= "width: 50%" aria- valuenow="25" aria- valuemin="0" aria- valuemax="100" ></div >'
        + '</div >'
        + '<div class="azMenuItem">'
        + '<select class="azureSelector" id= "azureLanguageTo"> Language</select > '
        + '</div >'
        + '<div class="azMenuItem">'
        + '<button type="button" class="btn btn-default" id= "azureFileViewer">File Viewer</button > '
        + '</div >'
        + '</div>'
